...
var gunPeer = Gun(["https://fwee-gun-relay-node.herokuapp.com/gun"]);
...
export const MessageHandler = (props) => {
  ...
  function reducer(curr, receivedMessage) {
    switch (receivedMessage.type) {
      case "replace":
        receivedMessage.replacer.sort(
          (b, a) => a.time.valueOf() - b.time.valueOf()
        );
  ...
  useEffect(() => {
    ...
    if (!user.is) {
      setFirstTime(true);
      user.auth(
        "fweeMessageChainSecond",
        process.env.REACT_APP_TRANSACTION_SYSTEM_API_KEY
      );
  ...
  useEffect(() => {
    ...
    const delayDebounceFn = setTimeout(() => {
      if (!valid) {
        ...
        if (firstTime) props.handler();
      }
    }, 2000);

    return () => clearTimeout(delayDebounceFn);
  }, [firstTime, props, tempState.messageArray]);
